...
{:else if ($user.isSuccess && userData)}
    <main class="m-auto card w-full sm:w-5/6 mt-5 p-4 variant-soft flex flex-col items-center">
        <AvatarWithName src={avatarSrc} name={userData.name} email={userData.email} ></AvatarWithName>
        <slot></slot>
    </main>
{/if}